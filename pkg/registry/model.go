...
type Parameters struct {
	...
	MaxTokens   *int           `yaml:"max_tokens,omitempty" mapstructure:"max_tokens,omitempty"`
	Extra       map[string]any `yaml:",inline" mapstructure:",remain"`
}

func (p Parameters) Copy() Parameters {
	copied := Parameters{
		Temperature: CopyPtr(p.Temperature),
		MaxTokens:   CopyPtr(p.MaxTokens),
	}
...
func (p *Parameters) Merge(override *Parameters) {
	...
	if override.Temperature != nil {
		p.Temperature = CopyPtr(override.Temperature)
	}
	if override.MaxTokens != nil {
		p.MaxTokens = CopyPtr(override.MaxTokens)
	}
